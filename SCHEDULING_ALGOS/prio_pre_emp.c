...
// Function to implement the Priority Scheduling (Preemptive) algorithm
void priority_preemptive(struct Process processes[], int num_processes)
{
    ...
    while (completed_count < num_processes)
    {
        ...
        // Find the process with the highest priority (highest priority value) that has arrived and is not completed
        for (int i = 0; i < num_processes; i++)
        {
...
int main()
{
    ...
    priority_preemptive(processes, num_processes);
